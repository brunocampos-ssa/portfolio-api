...
Estes exercicios ficam em cima do Modulo 2: eles **nao pedem** que voce
reescreva o watcher ou o snapshot runner. Em vez disso, voce escrevera
testes, fixtures, mocks e benchmarks em torno do codigo que ja existe.

Antes de comecar, garanta que a base esta verde:
...
| #   | Titulo | Dificuldade | Conceitos-Chave |
|-----|--------|-------------|-----------------|
| M3.1 | Sub-tests com table-driven para o normalizador | Iniciante | `t.Run`, table tests, `testify/require` |
| M3.2 | Mock de `BalanceProvider` com `testify/mock` | Iniciante | `mock.On`, `mock.Anything`, `AssertExpectations` |
| M3.3 | Teste de cancelamento por contexto no `snapshot.Runner` | Intermediario | `context.WithCancel`, `time.AfterFunc`, `atomic.Int32` |
| M3.4 | Teste de integracao com `anvil_setBalance` | Intermediario | `testcontainers-go`, mutacao de chain, asserts em DB |
| M3.5 | Benchmark e profile do normalizador | Intermediario | `go test -bench`, `-cpuprofile`, `go tool pprof` |
| M3.6 | `go test -race` em um bug plantado propositalmente | Intermediario | Race detector, `sync/atomic`, leitura de stack trace |
| M3.7 | Impersonate + transferencia simulada no Anvil | Avancado | `anvil_impersonateAccount`, `eth_sendTransaction`, integracao completa |

---

## Exercicio M3.1: Sub-tests com table-driven para o normalizador

**Dificuldade:** Iniciante

### Objetivo

Adicionar uma tabela de casos extra em
`internal/watcher/normalizer_extra_test.go` cobrindo pelo menos **quatro
novos cenarios** de `NormalizeTransferLog`.

### Conceitos envolvidos

- Testes de tabela em Go (`[]struct { name ...; want ... }`).
- Sub-tests com `t.Run(tt.name, func(t *testing.T) {...})` -- cada caso
  aparece como teste individual no output, o que facilita depurar falhas.
- `testify/require` para pre-condicoes, `testify/assert` para checagens
  independentes.

### Arquivos sugeridos

- `internal/watcher/normalizer_extra_test.go` (adicionar casos a
  `TestNormalizeTransferLog_TableDriven`).

### Criterios de aceite

- Pelo menos 4 novos casos adicionados (ex: `BlockNumber=""`,
  `Address` desconhecido, `Removed=true` + tracked, tracked como `from`
  **e** `to`).
- Todos os testes passam com `go test ./internal/watcher/... -v -count=1`.
- Os nomes dos cenarios (campo `name`) sao descritivos; nada de `case1`.

---

## Exercicio M3.2: Mock de `BalanceProvider` com testify/mock

**Dificuldade:** Iniciante

### Objetivo

Usar `internal/testutil/mocks` para testar o caminho feliz e um caminho
de erro do `snapshot.Runner` **sem** subir containers.

### Conceitos envolvidos

- `testify/mock`: `On`, `Return`, `Run`, `AssertExpectations`, `Maybe`.
- Diferenca entre stubs manuais (como em `runner_test.go`) e mocks
  (como em `runner_extra_test.go`).

### Instrucoes

1. Crie um novo arquivo `internal/snapshot/runner_my_test.go`.
2. Escreva dois testes:
   - `TestRunner_Run_PriceProviderRetriesTwice` -- o mock do price
     provider retorna erro nas duas primeiras chamadas e um preco valido
     na terceira. Verifique que a chamada final foi feita com
     `priceProvider.AssertNumberOfCalls(t, "GetPriceUSD", 3)`.
     (Dica: a implementacao atual nao faz retry -- voce vai observar
     que o teste falha com `AssertExpectations`. Documente o
     comportamento atual em um comentario; nao precisa implementar retry.)
   - `TestRunner_Run_NoTokenProvider_OnlyNativeItems` -- passe `nil`
     como `tokenProvider` e verifique que nenhum `CreateSnapshotItem` com
     `AssetType = "erc20"` e chamado.
...
- Os testes cobrem os dois cenarios descritos.
- Os mocks usam `mock.On` com matchers adequados (`mock.Anything`
  quando nao importa; valores concretos quando importa).
- `go test ./internal/snapshot/... -v` passa (com as observacoes do
  primeiro teste, se o retry nao estiver implementado).

---

## Exercicio M3.3: Teste de cancelamento por contexto no `snapshot.Runner`

**Dificuldade:** Intermediario

### Objetivo

Escrever um teste que prova que o `snapshot.Runner` **para de processar
wallets** assim que o contexto e cancelado, em vez de terminar a fila.

### Conceitos envolvidos

- `context.WithCancel`, `time.AfterFunc`.
- `sync/atomic` para contar chamadas sem race condition.
- O contrato do Go: **cancelar contexto nao mata goroutines**, apenas
  sinaliza. Quem usa `ctx.Done()` corretamente respeita o cancel.

### Instrucoes

1. Siga o padrao de `TestRunner_Run_ContextCancellation` em
   `runner_extra_test.go`. Construa um cenario com 20 wallets e 2
   workers.
2. Configure `BalanceProvider.GetBalance` para bloquear por 100ms
   observando `ctx.Done()`.
3. Use `time.AfterFunc(150*time.Millisecond, cancel)` para cancelar
   apos um unico lote de workers ter terminado.
4. Assere que `balanceCalls.Load() < 20`.

### Arquivos sugeridos

- Adicione uma variante em `runner_extra_test.go` (ou novo arquivo).

### Criterios de aceite

- O teste passa com `-race`: `go test ./internal/snapshot/... -race -v`.
- O teste falha se voce comentar os `select { case <-ctx.Done(): return }`
  internos do worker pool (tente!).

---

## Exercicio M3.4: Teste de integracao com `anvil_setBalance`

**Dificuldade:** Intermediario

### Objetivo

Escrever um novo teste **de integracao** que:

1. Usa `ethutil.SetEthBalance` para depositar 0 ETH em um novo endereco.
2. Executa o `snapshot.Runner` completo.
3. Verifica que o item do DB tem `amount = 0` e `usd_value = 0`.

### Conceitos envolvidos

- `testcontainers-go` + build tag `integration`.
- Helpers em `internal/testutil/ethutil`.
- Assercoes diretas no Postgres real (via `env.DB.QueryRowContext`).

### Instrucoes

1. Crie `test/integration/snapshot_zero_balance_test.go`.
2. Copie o formato de `snapshot_integration_test.go`: build tag,
   package, imports.
3. Seed um usuario + uma wallet (`seedUserAndWallet`), depois use
   `ethutil.SetEthBalance(ctx, rpc, addr, "0x0")` para garantir zero.
4. Rode o runner, depois consulte a tabela:
   ```go
   ...
   ```
5. Assere que ambos sao zero.

### Criterios de aceite

- `make test-integration` roda o teste novo e ele passa.
- Se voce comentar a linha `SetEthBalance` e usar
  `ethutil.EthToWei(1)`, o teste falha corretamente.

---

## Exercicio M3.5: Benchmark e profile do normalizador

**Dificuldade:** Intermediario

### Objetivo

Identificar o hot path de `NormalizeTransferLog` usando `pprof` e propor
uma otimizacao (nao e necessario implementar).

### Conceitos envolvidos

- `go test -bench` + `-cpuprofile`, `-memprofile`.
- `go tool pprof`, especialmente `-top` e `-list`.
- Interpretacao de `flat%` vs `cum%`.
...
2. Analise:
   ...
   ```
3. Em um comentario no topo de `normalizer_extra_test.go`, escreva 3
   bullet points:
   - Qual funcao consome mais CPU dentro do normalizador?
   - Qual alocacao mais custosa?
   - Uma proposta de otimizacao (ex: reuso de `big.Int` via `sync.Pool`;
     cache de `strings.ToLower`; etc.).

### Criterios de aceite

- O benchmark roda e produz `cpu.out`.
- O comentario esta presente, especifico e baseado em evidencia dos
  outputs do pprof.

---

## Exercicio M3.6: `go test -race` em um bug plantado propositalmente

**Dificuldade:** Intermediario

### Objetivo

Plantar um bug de race condition dentro do `watcher.metricsWorker` e
provar que o detector de corrida do Go o encontra.

### Conceitos envolvidos

- Race detector do Go (`-race`).
- Quando `int` **nao** e seguro para acesso concorrente.
- `sync/atomic.Int64` como solucao.

### Instrucoes

1. Em `internal/watcher/watcher.go`, no `metricsWorker`, substitua as
   variaveis `incoming` e `outgoing` por um campo compartilhado fora
   da goroutine (crie uma struct `metrics` exportada com contadores
   `int64` simples -- **sem atomic**).
2. Em um novo teste em `internal/watcher/watcher_race_test.go`, crie um
   segundo goroutine que le os contadores enquanto o watcher os
   incrementa. Rode:
   ```bash
   go test ./internal/watcher/... -race -run TestWatcherMetricsRace
   ```
3. Observe o output `WARNING: DATA RACE`. Copie a stack trace para um
   comentario no teste.
4. Corrija usando `atomic.Int64` e prove que agora o `-race` fica limpo.
5. **No final, reverta todas as alteracoes em producao** -- o codigo
   original nao tinha o bug. O exercicio e didatico.

### Criterios de aceite

- Antes da correcao: `-race` reporta data race com stack trace
  apontando para o campo compartilhado.
- Depois da correcao: `-race` passa.
- O exercicio reverte as mudancas de producao e mantem apenas o teste
  (ajustado para usar os contadores originais) em um branch separado.

---

## Exercicio M3.7: Impersonate + transferencia simulada no Anvil

**Dificuldade:** Avancado

### Objetivo

Usar `ethutil.Impersonate` + `eth_sendTransaction` para simular uma
transferencia on-chain entre duas wallets, depois validar que o watcher
(rodando contra Anvil) persiste o evento no Postgres.

### Conceitos envolvidos

- `anvil_impersonateAccount` -- atuar como um endereco sem chave privada.
- `eth_sendTransaction` via `ethutil.Client.Call`.
- Integracao ponta a ponta: watcher + Postgres + Anvil.

### Instrucoes

1. Em `test/integration/watcher_integration_test.go` (novo arquivo com
   `//go:build integration`):
   - Seed de uma wallet rastreada (`w-source`) com 10 ETH.
   - Impersonate `w-source`.
   - Chame `eth_sendTransaction` com `from=w-source`, `to=w-dest`,
     `value=0x1`.
   - Inicie `watcher.NewWatcher(...).Run(ctx)` em uma goroutine.
   - Espere ate que o evento apareca na tabela `wallet_events` (poll
     com timeout razoavel, ex: 10s).
2. **Atencao:** o watcher atual s filtra **ERC-20 Transfer**, nao
   transfers nativas de ETH. Voce vai precisar deployar um contrato
   ERC-20 simples no Anvil (ou usar `anvil_setCode` para injetar um
   mock) e chamar `transfer` nele. Alternativamente, documente que
   ETH nativo nao e capturado pelo watcher atual e escreva o teste
   para um contrato ERC-20 predefinido.

### Criterios de aceite

- `make test-integration` passa.
- O teste valida no DB que um `wallet_events` com `direction=incoming`
  ou `outgoing` foi persistido.
- O teste limpa a chain / DB entre runs (idempotente).

---

## Dicas gerais -- Modulo 3

1. **Use `t.Context()`** em vez de `context.Background()` nos testes.
   Ele cancela automaticamente quando o teste termina, o que ajuda
   testcontainers a limpar containers orfaos.
2. **Evite `time.Sleep`** para sincronizar testes. Prefira canais,
   `sync.WaitGroup`, ou `require.Eventually(...)` do testify.
3. **Benchmarks nao sao um ranking.** `1ms/op` em um laptop M1 pode
   ser `3ms/op` em um CI x86. Use benchmarks para **comparar
   alternativas na mesma maquina**, nao para reportar numeros
   absolutos.
4. **Race detector nao e magica.** Ele so detecta races que de fato
   aconteceram durante a execucao do teste. Combine com testes de
   propriedade (contagens, somas) que falham mesmo se o race nao for
   observado.
