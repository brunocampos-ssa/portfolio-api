...
func (s *AuthService) Refresh(ctx context.Context, refreshToken, userAgent, ip string) (*Tokens, error) {
	...
	return s.issueTokens(ctx, row.UserID, userAgent, ip, row.ID)
}
...
// issueTokens mints a new access+refresh pair, persists the refresh row,
// and (when rotating from an existing token) links the old row to the new
// one via MarkRotated.
//
// The order of operations matters: insert-then-rotate, never the other
// way around. If we marked the old token rotated first and then crashed
// before inserting the new one, the user would lose access while the
// chain pointer pointed at nothing.
func (s *AuthService) issueTokens(ctx context.Context, userID, userAgent, ip, rotateFromID string) (*Tokens, error) {
	...
	if rotateFromID != "" {
		if err := s.refresh.MarkRotated(ctx, rotateFromID, row.ID, now); err != nil {
			return nil, domain.NewInternalError(op, err)
		}
	}
...
// dummyArgon2idHash is used by Login to keep timing constant when the
// email is unknown. It is the argon2id hash of "" — produced once via
// NewArgon2idHasher and pasted here. The hash is a deliberate constant
// so the verify cost is identical to a real verification regardless of
// whether the user exists.
//
// Hash of "do-not-use-this-password" with default params. Generated by
// running internal/auth/cmd/genhash (not committed) — kept as a
// hard-coded string so the constant-timing path doesn't depend on
// being able to run argon2 at startup.
